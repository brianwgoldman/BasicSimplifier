...
using std::cout;
vector<int> Knowledge::simplify(vector<int> clause, bool& unsat) const {
  for (size_t i=0; i < clause.size(); i++) {
    size_t variable = abs(clause[i]);
    bool is_true = clause[i] > 0;
    // See if this variable has been assigned
    auto assigned_it = assigned.find(variable);
    auto rewrote_it = rewrites.find(variable);
    if (assigned_it != assigned.end()) {
      if (is_true == assigned_it->second) {
        return {};
      } else {
    ...
    } else if (rewrote_it != rewrites.end()) {
      bool has_both = false;
      for (size_t c=0; c < clause.size(); c++) {
        if (static_cast<size_t>(abs(clause[c])) == rewrote_it->second.to) {
          has_both = true;
          bool negated = is_true != (clause[c] > 0);
          if (negated != rewrote_it->second.negated) {
            return {};
          }
          break;
        }
      ...
      if (not has_both) {
        clause[i] = rewrote_it->second.to;
        if (is_true == rewrote_it->second.negated) {
          clause[i] = -clause[i];
        }
      } else {
        std::swap(clause[i], clause.back());
